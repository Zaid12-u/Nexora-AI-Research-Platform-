...
export default function GapFinder() {
  ...
  const [query, setQuery] = useState(
    localStorage.getItem('lastGapQuery') || ''
  )
  const [loading, setLoading] = useState(false)
  const [result, setResult] = useState(
    JSON.parse(localStorage.getItem('lastGapResult') || 'null')
  )
  const [error, setError] = useState('')
  const [searched, setSearched] = useState(
    localStorage.getItem('lastGapResult') ? true : false
  )
  ...
  const handleSearch = async (e) => {
    ...
    try {
      const res = await axios.post(
        'http://localhost:8000/gap-finder',
        { query }
      )
      ...
      await axios.post(
        ...
        {
          query: `[Gap Finder] ${query}`,
          results: res.data.papers
        },
        {
          headers: {
            ...
            <div className="bg-[#111] border-l-2 border-emerald-500 rounded-xl p-5">
              <div className="flex items-center justify-between mb-3">
                ...
                <span className="text-gray-600 text-xs">
                  {result.papers_analyzed} papers analyzed
                </span>
              </div>
                ...
                {result.papers?.map((paper, i) => (
                  ...
                  >
                    <div className="flex items-start gap-3">
                      <span className="text-emerald-500 font-bold text-xs mt-0.5 w-5 flex-shrink-0">
                        {i + 1}
                      </span>
        ...
        {!searched && (
          <div className="text-center py-24">
            ...
            <p className="text-gray-600 text-sm">
              Enter a topic to discover what's missing in research
            </p>
          </div>
        )}
